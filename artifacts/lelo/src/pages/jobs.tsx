...
import { Briefcase, CheckCircle2, Loader2, Paperclip, Send, X } from "lucide-react";
...
export default function JobsPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (name.trim().length < 2) {
      toast({ title: t("الاسم مطلوب", "Name is required"), variant: "destructive" });
      return;
    }
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      toast({ title: t("الإيميل غير صحيح", "Invalid email"), variant: "destructive" });
      return;
    }
    if (qualifications.trim().length < 10) {
      toast({ title: t("اكتب مؤهلاتك بالتفصيل", "Please describe your qualifications"), variant: "destructive" });
      return;
    }
    ...
    try {
      await apiFetch("/api/jobs/apply", {
        method: "POST",
        body: JSON.stringify({ name: name.trim(), email: email.trim(), qualifications: qualifications.trim(), cvUrl }),
      });
      setSubmitted(true);
    } catch (err) {
      toast({
        title: t("خطأ في الإرسال", "Submission error"),
        description: err instanceof Error ? err.message : t("حاول تاني.", "Please try again."),
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
        ...
        <section className="py-12 px-4">
          <div className="container mx-auto max-w-2xl">
            <div className="text-center mb-10">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-3" style={{ color: "var(--gold-light)" }}>
                {t("الوظائف", "Jobs")}
              </h1>
              ...
              <Card className="border-border/40 bg-background/60 backdrop-blur">
                <CardContent className="p-10 text-center">
                  ...
                  <h2 className="text-2xl font-bold mb-3">
                    {t("تم إرسال طلبك!", "Application Received!")}
                  </h2>
                  ...
                  <form onSubmit={onSubmit} className="grid gap-5">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        disabled={busy}
                        placeholder={t(
                          "اكتب مؤهلاتك الدراسية وخبراتك في مجال العقارات أو أي مجال تاني...",
                          "Describe your educational background and experience in real estate or any other relevant field...",
                        )}
                      />
                    </div>
                    ...
                    <div className="grid gap-2">
                      ...
                      >
                        ...
                        <span className="text-sm text-foreground/70 flex-1 truncate">
                          {cvFile
                            ? cvFile.name
                            : t("اضغط لرفع ملف PDF أو Word", "Click to upload PDF or Word file")}
                        </span>
                        {cvFile && !busy && (
                          <button
                            type="button"
                        ...
                        onChange={(e) => {
                          ...
                          if (f) {
                            if (f.size > 10 * 1024 * 1024) {
                              toast({ title: t("الملف كبير (أقصى 10MB)", "File too large (max 10MB)"), variant: "destructive" });
                              return;
                            }
                            setCvFile(f);
                          }
                      ...
                      <p className="text-xs text-foreground/40">
                        {t("PDF أو Word — حجم أقصى 10MB", "PDF or Word — max 10MB")}
                      </p>
                    </div>
                    ...
                    <Button
                      type="submit"
                      disabled={busy}
                      className="rounded-xl text-black font-semibold w-full mt-2"
                      style={{ background: "var(--gold)" }}
                    >
                      {busy ? (
                        <>
                          <Loader2 className={`${isAr ? "ml-2" : "mr-2"} h-4 w-4 animate-spin`} />
                          {uploading ? t("جاري رفع السي في...", "Uploading CV...") : t("جاري الإرسال...", "Sending...")}
                        </>
                      ) : (
                        <>
